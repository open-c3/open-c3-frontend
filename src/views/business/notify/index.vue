...
<template>
  <div class="body-layout1">
    ...
    <search-from :params="params" :config="config" @reset="reset" @query="getList">
      <template #name>
        <span class="df">
          <el-input v-model="params.name" :placeholder="$t('creatorFullName')"/>
    ...
    <div class="df jc_fe">
      <el-input v-model.trim="user" :placeholder="$t('addAlarmUser')" class="w500">
        <template #append>
          <el-button :disabled="user === ''" @click="addFun" icon="user" type="primary">{{ $t('addAlarmUser') }}</el-button>
        </template>
      </el-input>
    </div>
...
import { getNotifyList, addNotifyUser, deleteNotify } from '@/api/business/notify'
export default {
  ...
  setup () {
    ...
    const setCreateUser = () => {
      state.params.name = store.getters.userInfo?.email
    }
    ...
    const getList = () => {
      ...
      getNotifyList(treeId.value, getNotEmptyObj(state.params)).then((res: any) => {
    ...
    const deleteFun = (id: string | number) => {
      ElMessageBox.confirm(proxy.$t('deleteConfirmDesc'), '', {
        ...
        state.tableConfig.loading = true
        deleteNotify(treeId.value, id).then(res => {
    ...
    // 添加机器
    const addFun = () => {
      addNotifyUser(treeId.value, {user: state.user}).then(res => {
        proxy.$notification('operationSuccess')
        state.user = ''
        getList()
      })
