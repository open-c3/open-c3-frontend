<template>
  <div class="body-layout1">
    ...
    <search-from :params="params" :config="config" @reset="searchReset" @query="handleSearch">
      <template #user>
        <div class="df">
          <el-input v-model="params.user" :placeholder="$t('creatorFullName')" />
...
import {
  ...
  filterVersion,
  START_METHOD_MAP,
  JOBX_LIST_SEARCH_CONFIG,
  JOBX_LIST_TABLE_CONFIG,
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      ...
      config: JOBX_LIST_SEARCH_CONFIG,
      tableConfig: {
        loading: false,
        thead: JOBX_LIST_TABLE_CONFIG,
        total: 0,
        list: [],
        allList: [],
        page: 1,
    ...
    const getData: (param?: ISearchFormParams) => Promise<void> = async (param?: ISearchFormParams): Promise<void> => {
      ...
      const dataRet = await getJobxGroupList(params).catch(err => { state.tableConfig.loading = false })
      if (dataRet) {
    ...
    const operate: (type: string, row: any) => void = (type: string, row: any):void => {
      if (type === 'detail') {
        router.push(`/history/jobxdetail/${treeId.value}/${row.name.split('_')[2]}/${row.uuid}`)
      }
    }
