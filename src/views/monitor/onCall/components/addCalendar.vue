...
import { ADD_CALENDAR_CONFIG } from '../config'
import { getCalendar, getConfigOnCallById } from '@/api/monitor/onCall.js'
...
export default {
  ...
  setup (prop, context) {
    const state = reactive({
      ...
      config: ADD_CALENDAR_CONFIG
    })
    const editorInit = (editor) => {
      editor.setReadOnly(true)
    }
    const getConfig = () => {
      getCalendar(prop.groupInfo.name, state.params.username).then((res: any) => {
    ...
    const getUser = () => {
      ...
      getConfigOnCallById(prop.groupInfo.id).then((res: any) => {
        ...
        state.config.form.config[2].data = user
      })
    }
    ...
    watch(() => state.params.username, (newVal, oldVal) => {
      getConfig()
    }, {
      deep: true
    })
