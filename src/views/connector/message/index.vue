...
import { getMessage } from '@/api/connector/mailMessage'
export default {
  ...
  setup () {
    ...
    const getFilterList = () => {
      state.tableConfig.filterList = state.tableConfig.allList.filter(item => item.time.includes(state.params.time) && item.mesg.includes(state.params.mesg))
